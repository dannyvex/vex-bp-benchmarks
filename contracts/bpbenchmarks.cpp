#include <bpbenchmarks.hpp>

ACTION bpbenchmarks::cpu() {
...
BOOL bpbenchmarks::is_prime(int p) {
    if (p == 2) {
        return TRUE;
    } else if (p <= 1 || p % 2 == 0) {
        return FALSE;
...
BOOL bpbenchmarks::is_mersenne_prime(int p) {
    if (p == 2) return TRUE;
...
EOSIO_DISPATCH(bpbenchmarks, (cpu))
